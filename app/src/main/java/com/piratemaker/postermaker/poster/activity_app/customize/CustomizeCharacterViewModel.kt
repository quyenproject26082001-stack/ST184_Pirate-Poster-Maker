...
import com.piratemaker.postermaker.poster.core.utils.key.AssetsKey
...
import com.piratemaker.postermaker.poster.data.model.custom.ItemColorImageModel
import com.piratemaker.postermaker.poster.data.model.custom.ItemColorModel
...
import com.piratemaker.postermaker.poster.data.model.custom.LayerTransformModel
...
class CustomizeCharacterViewModel : ViewModel() {
    ...
    suspend fun addValueToItemNavList() {
        ...
        _dataCustomize.value!!.layerList.forEachIndexed { index, layer ->
            ...
            Log.d("LayerCount", "[$index] folder=${layer.imageNavigation.substringAfterLast('/').removeSuffix("/").let { layer.imageNavigation.split("/").dropLast(1).last() }} | layerSize=${layer.layer.size} | itemCount=${itemNavList[index].size}")
        }
        Log.d("LayerCount", "=== TOTAL tabs=${itemNavList.size}, dataName=${_dataCustomize.value?.dataName} ===")
    }
...
// Extension other
    ...
    fun getSuggestionList(): SuggestionModel {
        return SuggestionModel(
            ...
            colorItemNavList = ArrayList(colorItemNavList.map { ArrayList(it.map { item -> item.copy() }) }),
            ...
            layerTransformList = ArrayList(layerTransformList.map {
                LayerTransformModel(it.translationX, it.translationY, it.rotation, it.scaleX, it.scaleY)
            })
        )
    }
