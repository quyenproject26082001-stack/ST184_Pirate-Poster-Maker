...
class CropView : AppCompatImageView {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        overlayPath.addRoundRect(cropRect, cornerRadius, cornerRadius, Path.Direction.CW)
    ...
    private fun initCropView() {
        ...
        overlayPaint.color = Color.argb(0x80, 0x96, 0x96, 0x96)
    ...
    fun isModified(): Boolean {
        val defaultX = (width - initial_size) / 2
        val defaultY = (height - initial_size) / 2
        return leftTop!!.x != defaultX || leftTop!!.y != defaultY ||
                rightBottom!!.x != defaultX + initial_size || rightBottom!!.y != defaultY + initial_size
    }
    ...
    private fun adjustRectangle(x: Int, y: Int) {
        when (activeSide) {
            ...
            DRAG -> {
                val dx = x - previous!!.x
                val dy = y - previous!!.y
                val newLtX = leftTop!!.x + dx
                val newLtY = leftTop!!.y + dy
                val newRbX = rightBottom!!.x + dx
                val newRbY = rightBottom!!.y + dy
                if (newLtX >= 0 && newRbX <= width && newLtY >= 0 && newRbY <= height) {
                    leftTop!!.set(newLtX, newLtY)
                    rightBottom!!.set(newRbX, newRbY)
                }
            }
        }
    }
    ...
    private fun getAffectedSide(x: Float, y: Float): Int {
        val buffer = (40 * resources.displayMetrics.density).toInt()
